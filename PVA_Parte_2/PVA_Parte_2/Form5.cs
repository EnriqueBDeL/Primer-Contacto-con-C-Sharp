...
/*
    ...
    - Herramientas: Conectar bas de datos
    - Seleccionar Microsoft SQL Server y darle a continuar.
    - Escribimos en Nombre del servidor: (local)\SQLEXPRESS      (!!!NO LE DAMOS A ACTUALIZAR!!!)
    - Marcamos en confiar en el certificado de servidor.
    - Elegimos la BBDD master.
...
namespace PVA_Parte_2
{
    public partial class Form5 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            comandosql.CommandText = "SELECT Marca,Patrocinadores FROM Coches";
        ...
        {
            string competicion = comboBox1.SelectedItem?.ToString();
            ...
            {
                MessageBox.Show("Por favor, seleccione una opcion para buscar.");
                return;
            }

            string campoBuscar = competicion == "Marcas" ? "Marca" : "Patrocinadores";

            try
            {
                listView1.Items.Clear(); // Limpiar resultados anteriores

                conexion.Open();
                comandosql.Connection = conexion;
                comandosql.CommandText = $"SELECT Marca,Patrocinadores FROM Coches WHERE {campoBuscar} = @Coches";

                // Limpiar parámetros anteriores
                comandosql.Parameters.Clear();

                // Agregar nuevo parámetro
                comandosql.Parameters.AddWithValue("@Coches", palabraBuscar);
            ...
            {
                MessageBox.Show("Error al buscar la opcion: " + ex.Message);
            }
            finally
            {
                conexion.Close();
            ...
            if (modo == 0) // Si está en modo Añadir
            {
                ...
                if (!string.IsNullOrEmpty(palabraEspañol) && !string.IsNullOrEmpty(palabraIngles))
                {
                    try
                    {
                        ...
                        comandosql.Connection = conexion;
                        comandosql.Transaction = mitransaccion;
                        comandosql.CommandText = "INSERT INTO Coches (Marca, Patrocinadores) VALUES (@Marca, @Patrocinadores)";
                        comandosql.Parameters.AddWithValue("@Marca", palabraEspañol);
                        comandosql.Parameters.AddWithValue("@Patrocinadores", palabraIngles);
            ...
            else if (modo == 1) // Si está en modo Modificar
            {
                if (listView1.SelectedItems.Count > 0)
                {
                    ...
                    try
                    {
                        conexion.Open();
                        comandosql.Connection = conexion;
                        comandosql.CommandText = "UPDATE Coches SET Marca = @nuevoMarca, Patrocinadores = @nuevoPatrocinadores WHERE Marca = @viejoMarca AND Patrocinadores = @viejoPatrocinadores";
                        comandosql.Parameters.Clear(); // Limpiar los parámetros anteriores
                        comandosql.Parameters.AddWithValue("@nuevoMarca", nuevoPalabraEspañol);
                        comandosql.Parameters.AddWithValue("@nuevoPatrocinadores", nuevoPalabraIngles);
                        comandosql.Parameters.AddWithValue("@viejoMarca", selectedItem.SubItems[0].Text);
                        comandosql.Parameters.AddWithValue("@viejoPatrocinadores", selectedItem.SubItems[1].Text);
                        comandosql.ExecuteNonQuery();
                    }
                    catch (Exception ex)
                    {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                try
                {
                    conexion.Open();
                    comandosql.Connection = conexion;
                    comandosql.CommandText = "DELETE FROM Coches WHERE Marca = @Marca AND Patrocinadores = @Patrocinadores";
                    comandosql.Parameters.AddWithValue("@Marca", palabraEspañol);
                    comandosql.Parameters.AddWithValue("@Patrocinadores", palabraIngles);
                    comandosql.ExecuteNonQuery();
                }
